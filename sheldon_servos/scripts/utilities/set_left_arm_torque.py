...
from servo_joint_list import all_joints, head_joints, left_arm_joints
...
class SetTorque():
    def __init__(self, torque):
        ...
        for torque_enable in torque_services:
            if torque == 0.0:
                torque_enable(False) # if 0, set torque completely off!
            else:
                torque_enable(True)
...
if __name__=='__main__':
    ...
    rospy.init_node('set_left_arm_torque')
    ...
    if total > 1:
        ...
        print 'Setting Left Arm Torque to: ',  newTorque
    else:
